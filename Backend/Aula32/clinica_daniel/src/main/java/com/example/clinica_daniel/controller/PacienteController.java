...
//    PacienteDTO paciente, @RequestBody EnderecoDTO endereco
    ...
    public ResponseEntity<PacienteDTO> salvarPaciente(@RequestBody PacEndDTO pacEndDTO) {
        EnderecoDTO enderecoDTO = enderecoService.salvar(pacEndDTO.getEnderecoDTO());
        pacEndDTO.getPacienteDTO().setIdEndereco(enderecoDTO.getId());
        return ResponseEntity.ok(pacienteService.salvar(pacEndDTO.getPacienteDTO()));
    }
